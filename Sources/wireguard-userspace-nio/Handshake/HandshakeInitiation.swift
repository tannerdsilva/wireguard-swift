...
internal struct HandshakeInitiationMessage:Sendable {
    ...
    internal static func finalizeInitiationState(initiatorStaticPublicKey:UnsafePointer<PublicKey>, responderStaticPublicKey:UnsafePointer<PublicKey>, payload:Payload) throws -> AuthenticatedPayload {
        // step 14: msg.mac1 := MAC(HASH(LABEL-MAC1 || Spub(m')), msga)
        var hasher = try WGHasher()
        try hasher.update([UInt8]("mac1----".utf8))
        try hasher.update(responderStaticPublicKey)
        var hash = try hasher.finish()
        var mac1 = try wgMac(key: &hash, data: payload)
    ...
    internal struct AuthenticatedPayload:Sendable {
        let payload:Payload
