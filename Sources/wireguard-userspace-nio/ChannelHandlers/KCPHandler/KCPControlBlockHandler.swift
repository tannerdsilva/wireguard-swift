...
internal final class KCPLivePeer {
	...
	internal func handleChannelRead(context:ChannelHandlerContext, handler:KCPControlBlock.Handler, associatedSegment:PeerAssociated<KCPSegment>, now:NIODeadline, writeCounter:inout Int) {
		cbLoop: for i in 0..<controlBlocks.count {
			do {
				...
				try controlBlocks[i].handleChannelRead(context: context, handler: handler, associatedSegment: associatedSegment, now:now, writeCounter:&writeCounter)
				break cbLoop
			} catch {
				continue
			}
