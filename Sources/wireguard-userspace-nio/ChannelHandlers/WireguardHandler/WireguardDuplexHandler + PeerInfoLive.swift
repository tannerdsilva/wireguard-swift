...
extension PeerInfo {
	internal final class Live:@unchecked Sendable {
		...
		internal borrowing func getRecvVars(geometry inputPositionExplicit:Rotating<Session>.Positioned, now:NIODeadline) -> (nRecv:SlidingWindow<Counter>, tRecv:Result.Bytes32)? {
...
extension PeerInfo.Live {
	...
	internal borrowing func handshakeInitiationResponse(context:ChannelHandlerContext, now:NIODeadline, initiatorPeerIndex:PeerIndex) -> (initiatorEphemeralPrivateKey:MemoryGuarded<PrivateKey>, c:Result.Bytes32, h:Result.Bytes32, initiationPacket:Message.Initiation.Payload.Authenticated, handshakeBegin:NIODeadline)? {
...
extension PeerInfo.Live {
	...
	internal borrowing func session(forPeerM:PeerIndex) -> Rotating<Session>.Positioned? {
